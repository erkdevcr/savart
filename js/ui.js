...
const UI = (() => {
  ...
  function _buildAlbumCard(album) {
    ...
    card.innerHTML = `
      ...
      <div class="home-card-name">${escHtml(album.label || album.name)}</div>
      <div class="home-card-sub">${[album.artist, album.songCount ? album.songCount + ' canciones' : ''].filter(Boolean).map(escHtml).join(' · ')}</div>
    `;
